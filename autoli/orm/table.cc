...
std::string Table::getFilename() const {
    return autoli::ToLower(m_name + m_subfix);
}

bool Table::init(const tinyxml2::XMLElement& node) {
    if(!node.Attribute("name")) {
        AUTOLI_LOG_ERROR(g_logger) << "table name is null";
        return false;
    }

    m_name = node.Attribute("name");
    if(!node.Attribute("namespace")) {
        AUTOLI_LOG_ERROR(g_logger) << "table namespace is null";
        return false;
    }
    ...
    if(!cols) {
        AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " columns is null";
        return false;
    }

    const tinyxml2::XMLElement* col = cols->FirstChildElement("column");
    if(!col) {
        AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " column is null";
        return false;
    }
    ...
    do {
        ...
        if(!col_ptr->init(*col)) {
            AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " init column error";
            return false;
        }
        if(col_names.insert(col_ptr->getName()).second == false) {
            AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " column name="
    ...
    if(!idxs) {
        AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " indexs is null";
        return false;
    }

    const tinyxml2::XMLElement* idx = idxs->FirstChildElement("index");
    if(!idx) {
        AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " index is null";
        return false;
    }
    ...
    do {
        ...
        if(!idx_ptr->init(*idx)) {
            AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " index init error";
            return false;
        }
        if(idx_names.insert(idx_ptr->getName()).second == false) {
            AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " index name="
        ...
        if(idx_ptr->isPK()) {
            if(has_pk) {
                AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " more than one pk"; 
                return false;
            }
            has_pk = true;
        }
        ...
        for(auto& x : cnames) {
            if(col_names.count(x) == 0) {
                AUTOLI_LOG_ERROR(g_logger) << "table name=" << m_name << " idx="
...
void Table::gen(const std::string& path) {
    std::string p = path + "/" + autoli::replace(m_namespace, ".", "/");
    autoli::FSUtil::Mkdir(p);
    gen_inc(p);
    gen_src(p);
}
...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"autoli/db/db.h", "autoli/util.h"};
    for(auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
    ...
    std::vector<std::string> ns = autoli::split(m_namespace, '.');
...
std::string Table::genToStringSrc(const std::string& class_name) {
    ...
    for(auto it = m_cols.begin();
        ...
        if((*it)->getDType() == Column::TYPE_UINT64
            ...
            ss << "autoli::Time2Str(" << GetAsMemberName((*it)->getName()) << ")"
               << ";" << std::endl;
        } else {
    ...
    ss << "    return autoli::JsonUtil::ToString(jvalue);" << std::endl;
    ss << "}" << std::endl;
    return ss.str();
}
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"autoli/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static autoli::Logger::ptr g_logger = AUTOLI_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v) \
    ...
    ofs << "        AUTOLI_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl << \
        "                 << \" errno=\"" \
