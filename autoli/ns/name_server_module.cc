...
bool NameServerModule::handleRockRequest(autoli::RockRequest::ptr request
                                         ,autoli::RockResponse::ptr response
                                         ,autoli::RockStream::ptr stream) {
    autoli::Atomic::addFetch(s_request_count, 1);
    switch(request->getCmd()) {
        ...
        default:
            AUTOLI_LOG_WARN(g_logger) << "invalid cmd=0x" << std::hex << request->getCmd();
            break;
    }
    return true;
}

bool NameServerModule::handleRockNotify(autoli::RockNotify::ptr notify
                                        ,autoli::RockStream::ptr stream) {
    return true;
}

bool NameServerModule::onConnect(autoli::Stream::ptr stream) {
    autoli::Atomic::addFetch(s_on_connect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    }
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        AUTOLI_LOG_INFO(g_logger) << "onConnect: " << *addr;
    }
    return true;
}

bool NameServerModule::onDisconnect(autoli::Stream::ptr stream) {
    autoli::Atomic::addFetch(s_on_disconnect, 1);
    auto rockstream = std::dynamic_pointer_cast<RockStream>(stream);
    if(!rockstream) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid stream";
        return false;
    ...
    auto addr = rockstream->getRemoteAddress();
    if(addr) {
        AUTOLI_LOG_INFO(g_logger) << "onDisconnect: " << *addr;
    }
...
NSClientInfo::ptr NameServerModule::get(autoli::RockStream::ptr rs) {
    autoli::RWMutex::ReadLock lock(m_mutex);
    auto it = m_sessions.find(rs);
    return it == m_sessions.end() ? nullptr : it->second;
}

bool NameServerModule::handleRegister(autoli::RockRequest::ptr request
                                      ,autoli::RockResponse::ptr response
                                      ,autoli::RockStream::ptr stream) {
    auto rr = request->getAsPB<RegisterRequest>();
    if(!rr) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid register request from: "
...
void NameServerModule::set(autoli::RockStream::ptr rs, NSClientInfo::ptr new_value) {
    if(!rs->isConnected()) {
        new_value = nullptr;
    }
    ...
    //autoli::WorkerMgr::GetInstance()->schedule("notify",
...
std::set<autoli::RockStream::ptr> NameServerModule::getStreams(const std::string& domain) {
    autoli::RWMutex::ReadLock lock(m_mutex);
    auto it = m_domainToSessions.find(domain);
    return it == m_domainToSessions.end() ? std::set<autoli::RockStream::ptr>() : it->second;
}
...
bool NameServerModule::handleQuery(autoli::RockRequest::ptr request
                                   ,autoli::RockResponse::ptr response
                                   ,autoli::RockStream::ptr stream) {
    auto qreq = request->getAsPB<QueryRequest>();
    if(!qreq) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid query request from: "
            << stream->getRemoteAddressString();
        return false;
    }
...
void NameServerModule::setQueryDomain(autoli::RockStream::ptr rs, const std::set<std::string>& ds) {
...
bool NameServerModule::handleTick(autoli::RockRequest::ptr request
                                  ,autoli::RockResponse::ptr response
                                  ,autoli::RockStream::ptr stream) {
    return true;
}
