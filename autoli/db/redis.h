#ifndef __AUTOLI_DB_REDIS_H__
#define __AUTOLI_DB_REDIS_H__
...
public:
    ...
    FoxRedis(autoli::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
private:
    struct FCtx {
        std::string cmd;
        autoli::Scheduler* scheduler;
        autoli::Fiber::ptr fiber;
        ReplyPtr rpy;
    };
...
public:
    ...
    FoxRedisCluster(autoli::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef autoli::Singleton<RedisManager> RedisMgr;
