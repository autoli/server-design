...
static autoli::ConfigVar<std::map<std::string, std::map<std::string, std::string> > >::ptr g_redis =
    autoli::Config::Lookup("redis.config", std::map<std::string, std::map<std::string, std::string> >(), "redis config");
...
Redis::Redis(const std::map<std::string, std::string>& conf) {
    ...
    m_port = autoli::TypeUtil::Atoi(tmp.substr(pos + 1));
    m_passwd = get_value(conf, "passwd");
    m_logEnable = autoli::TypeUtil::Atoi(get_value(conf, "log_enable", "1"));

    tmp = get_value(conf, "timeout_com");
    if(tmp.empty()) {
        tmp = get_value(conf, "timeout");
    }
    uint64_t v = autoli::TypeUtil::Atoi(tmp);
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
    ...
    if(c) {
        ...
        if(!m_passwd.empty()) {
            ...
            if(!r) {
                AUTOLI_LOG_ERROR(g_logger) << "auth error:("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(r->type != REDIS_REPLY_STATUS) {
                AUTOLI_LOG_ERROR(g_logger) << "auth reply type error:" << r->type << "("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(!r->str) {
                AUTOLI_LOG_ERROR(g_logger) << "auth reply str error: NULL("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
            if(strcmp(r->str, "OK") == 0) {
                ...
                AUTOLI_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
                    << m_host << ":" << m_port << ", " << m_name << ")";
                return false;
            }
        }
...
ReplyPtr Redis::cmd(const char* fmt, va_list ap) {
    ...
    if(!r) {
        if(m_logEnable) {
            AUTOLI_LOG_ERROR(g_logger) << "redisCommand error: (" << fmt << ")(" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
    ...
    if(!r) {
        if(m_logEnable) {
            AUTOLI_LOG_ERROR(g_logger) << "redisCommandArgv error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
        }
        return nullptr;
    }
    ReplyPtr rt(r, freeReplyObject);
...
ReplyPtr Redis::getReply() {
    ...
    if(m_logEnable) {
        AUTOLI_LOG_ERROR(g_logger) << "redisGetReply error: (" << m_host << ":" << m_port << ")(" << m_name << ")";
    }
    return nullptr;
}
...
FoxRedis::FoxRedis(autoli::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
...
void FoxRedis::OnAuthCb(redisAsyncContext* c, void* rp, void* priv) {
    ...
    if(strcmp(r->str, "OK") == 0) {
        AUTOLI_LOG_INFO(g_logger) << "auth ok: " << r->str << "("
            << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
    } else {
        AUTOLI_LOG_ERROR(g_logger) << "auth error: " << r->str << "("
            << fr->m_host << ":" << fr->m_port << ", " << fr->m_name << ")";
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if(!status) {
        AUTOLI_LOG_INFO(g_logger) << "FoxRedis::ConnectCb "
        ...
        if(!ar->m_passwd.empty()) {
            ...
            if(rt) {
                AUTOLI_LOG_ERROR(g_logger) << "FoxRedis Auth fail: " << rt;
            }
        }

    } else {
        AUTOLI_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb "
                    << c->c.tcp.host << ":" << c->c.tcp.port
                    << " fail, error:" << c->errstr;
...
void FoxRedis::DisconnectCb(const redisAsyncContext* c, int status) {
    AUTOLI_LOG_INFO(g_logger) << "FoxRedis::DisconnectCb "
               << c->c.tcp.host << ":" << c->c.tcp.port
               << " status:" << status;
    FoxRedis* ar = static_cast<FoxRedis*>(c->data);
    ar->m_status = UNCONNECTED;
...
void FoxRedis::CmdCb(redisAsyncContext *ac, void *r, void *privdata) {
    ...
    if(ctx->timeout) {
        delete ctx;
        //if(ctx && ctx->fiber) {
        //    AUTOLI_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd << "' timeout("
    ...
    if(ac->err) {
        if(m_logEnable) {
            autoli::replace(ctx->cmd, "\r\n", "\\r\\n");
            AUTOLI_LOG_ERROR(g_logger) << "redis cmd: '" << ctx->cmd
                        << "' "
                        << "(" << ac->err << ") " << ac->errstr;
        }
        if(ctx->fctx->fiber) {
...
//    AUTOLI_LOG_INFO(g_logger) << "DelayTimeCb";
...
bool FoxRedis::init() {
    if(m_thread == autoli::FoxThread::GetThis()) {
        return pinit();
    } else {
...
bool FoxRedis::pinit() {
    //AUTOLI_LOG_INFO(g_logger) << "pinit m_status=" << m_status;
    if(m_status != UNCONNECTED) {
        return true;
    }
    auto ctx = redisAsyncConnect(m_host.c_str(), m_port);
    if(!ctx) {
        AUTOLI_LOG_ERROR(g_logger) << "redisAsyncConnect (" << m_host << ":" << m_port
                    << ") null";
        return false;
    }
    if(ctx->err) {
        AUTOLI_LOG_ERROR(g_logger) << "Error:(" << ctx->err << ")" << ctx->errstr;
        return false;
    }
    ctx->data = this;
    ...
    m_context.reset(ctx, autoli::nop<redisAsyncContext>);
...
ReplyPtr FoxRedis::cmd(const char* fmt, va_list ap) {
    ...
    if(len == -1) {
        AUTOLI_LOG_ERROR(g_logger) << "redis fmt error: " << fmt;
        return nullptr;
    }
    ...
    //    free(buf);
    //}
    //ctx->scheduler = autoli::Scheduler::GetThis();
    //ctx->fiber = autoli::Fiber::GetThis();
    //ctx->thread = m_thread;

    FCtx fctx;
    fctx.cmd.append(buf, len);
    free(buf);
    fctx.scheduler = autoli::Scheduler::GetThis();
    fctx.fiber = autoli::Fiber::GetThis();
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
    ...
    do {
        ...
        if(len == -1 || !buf) {
            AUTOLI_LOG_ERROR(g_logger) << "redis fmt error";
            return nullptr;
        }
        fctx.cmd.append(buf, len);
        free(buf);
...
void FoxRedis::pcmd(FCtx* fctx) {
    if(m_status == UNCONNECTED) {
        AUTOLI_LOG_INFO(g_logger) << "redis (" << m_host << ":" << m_port << ") unconnected "
                   << fctx->cmd;
        init();
        if(fctx->fiber) {
...
FoxRedis::Ctx::Ctx(FoxRedis* r)
    ...
    //,scheduler(nullptr)
    ,thread(nullptr) {
    autoli::Atomic::addFetch(rds->m_ctxCount, 1);
}

FoxRedis::Ctx::~Ctx() {
    //cancelEvent();
    AUTOLI_ASSERT(thread == autoli::FoxThread::GetThis());
    //AUTOLI_ASSERT(destory == 0);
    autoli::Atomic::subFetch(rds->m_ctxCount, 1);
    //++destory;
    //cancelEvent();
    if(ev) {
        evtimer_del(ev);
...
void FoxRedis::Ctx::cancelEvent() {
    //if(ev) {
    //    if(thread == autoli::FoxThread::GetThis()) {
    //        evtimer_del(ev);
    //        event_free(ev);
    //    } else {
    //        auto e = ev;
...
void FoxRedis::Ctx::EventCb(int fd, short event, void* d) {
    ...
    if(ctx->rds->m_logEnable) {
        autoli::replace(ctx->cmd, "\r\n", "\\r\\n");
        AUTOLI_LOG_INFO(g_logger) << "redis cmd: '" << ctx->cmd << "' reach timeout "
                   << (ctx->rds->m_cmdTimeout.tv_sec * 1000
...
FoxRedisCluster::FoxRedisCluster(autoli::FoxThread* thr, const std::map<std::string, std::string>& conf)
    :m_thread(thr)
    ,m_status(UNCONNECTED)
    ,m_event(nullptr) {
    m_ctxCount = 0;
...
void FoxRedisCluster::ConnectCb(const redisAsyncContext* c, int status) {
    ...
    if(!status) {
        AUTOLI_LOG_INFO(g_logger) << "FoxRedisCluster::ConnectCb "
                   << c->c.tcp.host << ":" << c->c.tcp.port
                   << " success";
        if(!ar->m_passwd.empty()) {
            int rt = redisClusterAsyncCommand(ar->m_context.get(), FoxRedisCluster::OnAuthCb, ar, "auth %s", ar->m_passwd.c_str());
            if(rt) {
                AUTOLI_LOG_ERROR(g_logger) << "FoxRedisCluster Auth fail: " << rt;
            }
        }
    } else {
        AUTOLI_LOG_ERROR(g_logger) << "FoxRedisCluster::ConnectCb "
                    << c->c.tcp.host << ":" << c->c.tcp.port
                    << " fail, error:" << c->errstr;
    }
}
...
void FoxRedisCluster::DisconnectCb(const redisAsyncContext* c, int status) {
    AUTOLI_LOG_INFO(g_logger) << "FoxRedisCluster::DisconnectCb "
               << c->c.tcp.host << ":" << c->c.tcp.port
               << " status:" << status;
}
...
bool FoxRedisCluster::pinit() {
    ...
    AUTOLI_LOG_INFO(g_logger) << "FoxRedisCluster pinit:" << m_host;
    ...
    if(!ctx) {
        AUTOLI_LOG_ERROR(g_logger) << "redisClusterAsyncConnect (" << m_host
                    << ") null";
        return false;
    }
    if(ctx->err) {
    ...
    m_status = CONNECTED;
    //m_context.reset(ctx, redisAsyncFree);
    m_context.reset(ctx, autoli::nop<redisClusterAsyncContext>);
...
void FoxRedisCluster::pcmd(FCtx* fctx) {
    if(m_status != CONNECTED) {
        AUTOLI_LOG_INFO(g_logger) << "redis (" << m_host << ") unconnected "
                   << fctx->cmd;
        init();
        if(fctx->fiber) {
            fctx->scheduler->schedule(&fctx->fiber);
...
void FoxRedisCluster::Ctx::cancelEvent() {
    //AUTOLI_LOG_INFO(g_logger) << "cancelEvent " << autoli::FoxThread::GetThis()
    //           << " - " << thread
    //           << " - " << autoli::IOManager::GetThis()
    //           << " - " << cancel_count;
    //if(thread != autoli::FoxThread::GetThis()) {
    //    AUTOLI_LOG_INFO(g_logger) << "cancelEvent " << autoli::FoxThread::GetThis()
    //               << " - " << thread
    ...
    //    //AUTOLI_LOG_INFO(g_logger) << "cancelEvent thread=" << thread << " " << thread->getId()
    //    //           << " this=" << autoli::FoxThread::GetThis();
    //    //AUTOLI_ASSERT(thread == autoli::FoxThread::GetThis());
    //}
    //AUTOLI_ASSERT(!autoli::IOManager::GetThis());
    ////if(autoli::Atomic::addFetch(cancel_count) > 1) {
    ////    return;
    ////}
    ////AUTOLI_ASSERT(!autoli::Fiber::GetThis());
    ////autoli::RWMutex::WriteLock lock(mutex);
    //if(++cancel_count > 1) {
    //    return;
    //}
    //if(ev) {
...
IRedis::ptr RedisManager::get(const std::string& name) {
    ...
    if(r->getType() == IRedis::FOX_REDIS
        ...
        return std::shared_ptr<IRedis>(r, autoli::nop<IRedis>);
    }
    lock.unlock();
...
void RedisManager::init() {
    ...
    for(auto& i : m_config) {
        ...
        auto pool = autoli::TypeUtil::Atoi(get_value(i.second, "pool"));
        auto passwd = get_value(i.second, "passwd");
        total += pool;
        for(int n = 0; n < pool; ++n) {
            if(type == "redis") {
                autoli::Redis* rds(new autoli::Redis(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                autoli::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                autoli::Atomic::addFetch(done, 1);
            } else if(type == "redis_cluster") {
                autoli::RedisCluster* rds(new autoli::RedisCluster(i.second));
                rds->connect();
                rds->setLastActiveTime(time(0));
                autoli::RWMutex::WriteLock lock(m_mutex);
                m_datas[i.first].push_back(rds);
                ...
                auto conf = i.second;
                auto name = i.first;
                autoli::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    autoli::FoxRedis* rds(new autoli::FoxRedis(autoli::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
                ...
                auto name = i.first;
                autoli::FoxThreadMgr::GetInstance()->dispatch("redis", [this, conf, name, &done](){
                    autoli::FoxRedisCluster* rds(new autoli::FoxRedisCluster(autoli::FoxThread::GetThis(), conf));
                    rds->init();
                    rds->setName(name);
