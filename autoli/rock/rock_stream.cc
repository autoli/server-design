...
static autoli::ConfigVar<std::unordered_map<std::string
    ,std::unordered_map<std::string, std::string> > >::ptr g_rock_services =
    autoli::Config::Lookup("rock_services", std::unordered_map<std::string
...
RockStream::RockStream(Socket::ptr sock)
    ...
    AUTOLI_LOG_DEBUG(g_logger) << "RockStream::RockStream "
        << this << " "
        << (sock ? sock->toString() : "");
}
...
RockResult::ptr RockStream::request(RockRequest::ptr req, uint32_t timeout_ms) {
    if(isConnected()) {
        ...
        addCtx(ctx);
        uint64_t ts = autoli::GetCurrentMS();
        ctx->timer = autoli::IOManager::GetThis()->addTimer(timeout_ms,
                std::bind(&RockStream::onTimeOut, shared_from_this(), ctx));
        enqueue(ctx);
        autoli::Fiber::YieldToHold();
        return std::make_shared<RockResult>(ctx->result, autoli::GetCurrentMS() - ts, ctx->response, req);
    } else {
...
AsyncSocketStream::Ctx::ptr RockStream::doRecv() {
    //AUTOLI_LOG_INFO(g_logger) << "doRecv " << this;
    ...
    if(type == Message::RESPONSE) {
        ...
        if(!rsp) {
            AUTOLI_LOG_WARN(g_logger) << "RockStream doRecv response not RockResponse: "
                << msg->toString();
            return nullptr;
        }
        RockCtx::ptr ctx = getAndDelCtxAs<RockCtx>(rsp->getSn());
        if(!ctx) {
            AUTOLI_LOG_WARN(g_logger) << "RockStream request timeout reponse="
                << rsp->toString();
            return nullptr;
        }
        ...
        if(!req) {
            AUTOLI_LOG_WARN(g_logger) << "RockStream doRecv request not RockRequest: "
                << msg->toString();
            return nullptr;
        }
        if(m_requestHandler) {
            ...
            AUTOLI_LOG_WARN(g_logger) << "unhandle request " << req->toString();
        }
    } else if(type == Message::NOTIFY) {
        auto nty = std::dynamic_pointer_cast<RockNotify>(msg);
        if(!nty) {
            AUTOLI_LOG_WARN(g_logger) << "RockStream doRecv notify not RockNotify: "
                << msg->toString();
            return nullptr;
        }
        ...
        if(m_notifyHandler) {
            ...
            AUTOLI_LOG_WARN(g_logger) << "unhandle notify " << nty->toString();
        }
    } else {
        AUTOLI_LOG_WARN(g_logger) << "RockStream recv unknow type=" << type
            << " msg: " << msg->toString();
    }
    return nullptr;
}

void RockStream::handleRequest(autoli::RockRequest::ptr req) {
    autoli::RockResponse::ptr rsp = req->createResponse();
...
void RockStream::handleNotify(autoli::RockNotify::ptr nty) {
...
bool RockConnection::connect(autoli::Address::ptr addr) {
    m_socket = autoli::Socket::CreateTCP(addr);
...
static SocketStream::ptr create_rock_stream(ServiceItemInfo::ptr info) {
    autoli::IPAddress::ptr addr = autoli::Address::LookupAnyIPAddress(info->getIp());
    if(!addr) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid service info: " << info->toString();
        return nullptr;
    }
    ...
    autoli::WorkerMgr::GetInstance()->schedule("service_io", [conn, addr](){
        conn->connect(addr);
        conn->start();
    });
    return conn;
...
RockResult::ptr RockSDLoadBalance::request(const std::string& domain, const std::string& service,
    ...
    uint64_t ts2 = autoli::GetCurrentMS();
    if(r->result == 0) {
        stats.incOks(1);
