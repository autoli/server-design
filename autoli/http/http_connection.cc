...
HttpConnection::~HttpConnection() {
    AUTOLI_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            AUTOLI_LOG_DEBUG(g_logger) << "content_len=" << client_parser.content_len;
    ...
    if(!body.empty()) {
        ...
        AUTOLI_LOG_DEBUG(g_logger) << "content_encoding: " << content_encoding
            << " size=" << body.size();
...
HttpResult::ptr HttpConnection::DoRequest(HttpRequest::ptr req
    ...
    if(is_ssl) std::cout<<std::endl;//自己加的后面删除
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        AUTOLI_LOG_ERROR(g_logger) << "invalid uri=" << uri;
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = autoli::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            AUTOLI_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
       ...
       Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            AUTOLI_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            AUTOLI_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= autoli::GetCurrentMS())
