#ifndef __AUTOLI_WORKER_H__
#define __AUTOLI_WORKER_H__
...
public:
    ...
    static WorkerGroup::ptr Create(uint32_t batch_size, autoli::Scheduler* s = autoli::Scheduler::GetThis()) {
        return std::make_shared<WorkerGroup>(batch_size, s);
    }

    WorkerGroup(uint32_t batch_size, autoli::Scheduler* s = autoli::Scheduler::GetThis());
    ~WorkerGroup();
...
public:
    ...
    void schedule(const std::string& name, FiberOrCb fc, int thread = -1) {
        ...
        if(s) {
            ...
            static autoli::Logger::ptr s_logger = AUTOLI_LOG_NAME("system");
            AUTOLI_LOG_ERROR(s_logger) << "schedule name=" << name
                << " not exists";
        }
    }
...
typedef autoli::Singleton<WorkerManager> WorkerMgr;
