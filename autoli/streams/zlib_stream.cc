...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    AUTOLI_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    AUTOLI_ASSERT((window_bits >= 8 && window_bits <= 15));
    AUTOLI_ASSERT((memlevel >= 1 && memlevel <= 9));
...
autoli::ByteArray::ptr ZlibStream::getByteArray() {
