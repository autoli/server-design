...
static autoli::ConfigVar<std::string>::ptr g_module_path
...
bool Module::handleRequest(autoli::Message::ptr req
                           ,autoli::Message::ptr rsp
                           ,autoli::Stream::ptr stream) {
    AUTOLI_LOG_DEBUG(g_logger) << "handleRequest req=" << req->toString()
            << " rsp=" << rsp->toString() << " stream=" << stream;
    return true;
}

bool Module::handleNotify(autoli::Message::ptr notify
                          ,autoli::Stream::ptr stream) {
    AUTOLI_LOG_DEBUG(g_logger) << "handleNotify nty=" << notify->toString()
            << " stream=" << stream;
    return true;
}
...
bool Module::onConnect(autoli::Stream::ptr stream) {
    return true;
}

bool Module::onDisconnect(autoli::Stream::ptr stream) {
    return true;
}
...
bool RockModule::handleRequest(autoli::Message::ptr req
                               ,autoli::Message::ptr rsp
                               ,autoli::Stream::ptr stream) {
    auto rock_req = std::dynamic_pointer_cast<autoli::RockRequest>(req);
    auto rock_rsp = std::dynamic_pointer_cast<autoli::RockResponse>(rsp);
    auto rock_stream = std::dynamic_pointer_cast<autoli::RockStream>(stream);
    return handleRockRequest(rock_req, rock_rsp, rock_stream);
}

bool RockModule::handleNotify(autoli::Message::ptr notify
                              ,autoli::Stream::ptr stream) {
    auto rock_nty = std::dynamic_pointer_cast<autoli::RockNotify>(notify);
    auto rock_stream = std::dynamic_pointer_cast<autoli::RockStream>(stream);
...
void ModuleManager::init() {
    ...
    autoli::FSUtil::ListAllFile(files, path, ".so");
