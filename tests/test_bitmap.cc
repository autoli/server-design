...
#include "autoli/ds/bitmap.h"
#include "autoli/ds/roaring_bitmap.h"
...
void init(uint32_t size, std::set<uint32_t> v = {}) {
    ...
    std::set<uint32_t> v0;
    autoli::ds::Bitmap::ptr b;
    if(!size) {
        size = rand() % 4096;
        b.reset(new autoli::ds::Bitmap(size));
    ...
    autoli::ds::Bitmap::ptr c(new autoli::ds::Bitmap(size, 0xFF));
    *c &= *b;
    AUTOLI_ASSERT(c->getCount() == v0.size());
    ...
    //};
    //uint32_t size = 328;
    //autoli::ds::Bitmap::ptr b(new autoli::ds::Bitmap(size));
    //for(auto& i : vv) {
    //    b->set(i, true);
    //}
    ...
    autoli::ds::Bitmap::ptr bb(new autoli::ds::Bitmap(size, 0xFF));
    *b &= *bb;
#define XX(v) \
        AUTOLI_LOG_INFO(g_logger) << #v ": " << autoli::Join(v.begin(), v.end(), ",");
...
#define XX_ASSERT(a, b) \
    ...
    if(v2 != v4 || v1 != v3) {
        ...
        XX_ASSERT(v00, v3);
        XX_ASSERT(v000, v4);
        //AUTOLI_ASSERT(v00 == v3);
        //AUTOLI_ASSERT(v1 == v3);
        //AUTOLI_ASSERT(v2 == v4);
#undef XX_ASSERT
#undef XX
    }
    //AUTOLI_LOG_INFO(g_logger) << "size: " << size;
}
...
void test_compress(size_t size, std::set<uint32_t> v = {}) {
    ...
    if(*b != *bb) {
        AUTOLI_LOG_INFO(g_logger) << "b  :" << b->toString();
        AUTOLI_LOG_INFO(g_logger) << "bc :" << bc->toString();
        AUTOLI_LOG_INFO(g_logger) << "bb :" << bb->toString();
        AUTOLI_LOG_INFO(g_logger) << "size=" << size << " - " << autoli::Join(v.begin(), v.end(), ",");
...
void test_op(size_t size, std::set<uint32_t> v1, std::set<uint32_t> v2) {
    autoli::ds::Bitmap::ptr a;
    autoli::ds::Bitmap::ptr b;

    if(size == 0) {
        size = rand() % 4096;
        a.reset(new autoli::ds::Bitmap(size));
        b.reset(new autoli::ds::Bitmap(size));
    ...
    AUTOLI_ASSERT(a->cross(*b) == a->cross(*bc));
    ...
    AUTOLI_ASSERT(a->cross(*b) == (!and_sv.empty()));
...
int main(int argc, char** argv) {
    srand(time(0));
    {
        autoli::ds::Bitmap::ptr b(new autoli::ds::Bitmap(504));
    }
...
std::vector<autoli::ds::Bitmap::ptr> vs;
std::vector<autoli::ds::Bitmap::ptr> vs2;
std::vector<autoli::ds::RoaringBitmap::ptr> vs3;
...
void init() {
    ...
    vs3.resize(N);
    for(int i = 0; i < N; ++i) {
        autoli::ds::Bitmap::ptr r(new autoli::ds::Bitmap(M));
        autoli::ds::RoaringBitmap::ptr r2(new autoli::ds::RoaringBitmap());
        for(int i = 0; i < M / 1; ++i) {
            uint32_t v = rand() % M;
            r->set(v, true);
            r2->set(v, true);
...
void write_to_file(const std::string& name) {
    {
        ...
        a.reset(new autoli::ds::Bitmap(0));
        a->readFrom(ba);
...

    {
        ...
        a.reset(new autoli::ds::RoaringBitmap);
        a->readFrom(ba);
...
void load_from_file(const std::string& name) {
    ...
    ba->setPosition(0);
    autoli::ds::Bitmap::ptr a(new autoli::ds::Bitmap(0));
    a->readFrom(ba);
    std::cout << "compress= " << a->isCompress()
              << " rate=" << a->getCompressRate()
              << " size=" << a->getSize()
...
void test_uncompress2() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            autoli::ds::Bitmap::ptr b(new autoli::ds::Bitmap(M));
            *b |= *vs2[i];
        }
    }
}
    ...
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            ...
            *b |= *vs2[i];
            ...
            AUTOLI_ASSERT(*b == *vs2[i]->uncompress());
        }
    }
}
...
void test_uncompress4() {
    for(int i = 0; i < N; ++i) {
        for(int n = i + 1; n < N; ++n) {
            autoli::ds::Bitmap::ptr b(new autoli::ds::Bitmap(M, 0xff));
            *b &= *vs2[i];
...
void test_roaring_bitmap() {
    autoli::ds::RoaringBitmap::ptr rb(new autoli::ds::RoaringBitmap);
    for(int i = 0; i < 10; ++i) {
        rb->set(rand(), true);
    }
